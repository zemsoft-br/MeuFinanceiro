...
A validação ocorre antes de renderizar a forma decimal canônica, evitando que expoentes
extremos produzam material intermediário desnecessariamente grande.
...
Violações de unicidade são classificadas como conflito de identidade. Outras falhas
de integridade do banco são reduzidas a erro de persistência sanitizado, sem ecoar
valores financeiros ou parâmetros SQL.
