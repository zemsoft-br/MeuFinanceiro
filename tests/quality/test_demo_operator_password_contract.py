...
def test_compose_mounts_demo_operator_password_as_private_secret() -> None:
    ...
    assert "file: ./.demo/secrets/operator_password.txt" in compose
    assert "DEMO_OPERATOR_PASSWORD_FILE_HOST" not in compose
...
def test_demo_scripts_generate_reuse_purge_and_migrate_operator_password() -> None:
    ...
    assert "read_operator_password()" in unix
    assert 'raw.endswith("\\r\\n")' in unix
    assert 'raw.endswith(("\\n", "\\r"))' in unix
    ...
    assert "DEMO_OPERATOR_PASSWORD_FILE_HOST" not in unix
    assert "DEMO_OPERATOR_PASSWORD_FILE_HOST" not in windows
...
def test_demo_cli_prefers_single_explicit_password_source() -> None:
    ...
    assert "_read_operator_password_file" in cli
    assert 'raw.endswith("\\r\\n")' in cli
    assert ".strip()" not in cli
