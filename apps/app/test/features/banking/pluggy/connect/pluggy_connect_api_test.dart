...
void main() {
  test(
    'connect token response is strict and the secret wrapper is redacted',
    () async {
      final api = _api(
  ...
  test(
    'registration sends only transient itemId and accepts local result',
    () async {
      ...
      expect(
        ...
        '/api/v1/banking/pluggy/connections',
      );
  ...
  test(
    'registration rejects unknown status and extra provider payload',
    () async {
      for (final body in [
        '''
      {
  ...
  test(
    'registration rejects unsafe item identifiers before transport',
    () async {
